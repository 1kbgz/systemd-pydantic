...
lint-py:  ## lint python with ruff
	python -m ruff check systemd_pydantic
	python -m ruff format --check systemd_pydantic
...
fix-py:  ## autoformat python code with ruff
	python -m ruff check --fix systemd_pydantic
	python -m ruff format systemd_pydantic
...
test:  ## run python tests
	python -m pytest -v systemd_pydantic/tests

coverage:  ## run tests and collect test coverage
	python -m pytest -v systemd_pydantic/tests --cov=systemd_pydantic --cov-report term-missing --cov-report xml
